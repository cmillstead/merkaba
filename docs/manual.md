...
Mission Control is a React SPA served by FastAPI. Start it with `merkaba web` (default port 5173).

**Key files:** `web/app.py`, `web/routes/`, `web/src/` (React frontend)

### Sidebar Navigation

The sidebar organizes pages into four collapsible groups:

| Group | Pages |
|-------|-------|
| **Operations** | Dashboard, Tasks, Approvals, Kanban |
| **Knowledge** | Memory, Docs (placeholder) |
| **Team** | Chat, Agent Config (placeholder) |
| **System** | Analytics, Settings, Calendar |

Each group header can be clicked to expand or collapse its children. The sidebar also contains the notification bell icon with an unread badge.

### Dashboard View

The dashboard is the default landing page (route `/`). It provides an at-a-glance overview of system state:

- **KPI cards** -- Four summary cards at the top: active tasks, agents online, pending approvals, and WebSocket connection status. Each updates in real time via WebSocket.
- **Active agents panel** -- Lists currently running agents with status indicators.
- **Recent activity feed** -- Chronological feed of recent system events (task completions, approval decisions, worker runs).

The dashboard also links to the constellation, kanban, and diagnostics sub-views within Mission Control, toggled by keyboard shortcuts.

### Calendar View

The calendar page (`/calendar`) displays a weekly schedule of all registered workers:

- **CSS Grid layout** -- Seven columns (Mon--Sun) with time rows.
- **Month toggle** -- Navigate between months to see past and future schedules.
- **Cron parsing** -- Worker schedules defined as cron expressions are parsed and rendered as colored time blocks.
- **Color coding** -- Each worker type has a distinct color for visual differentiation.
- **Always On banner** -- Workers with very frequent schedules (e.g., every minute) display a banner instead of individual blocks.
- **Click-to-trigger popover** -- Clicking a worker block opens a popover with details and a button to trigger the worker immediately.

### Kanban Board

The kanban board is available as a standalone page at `/kanban` (promoted from a Mission Control sub-view):

- Columns represent task states: pending, in progress, completed, failed.
- Cards show task name, worker type, and last run timestamp.
- **Worker type filter** -- A dropdown filters the board to show only tasks for a specific worker type.
- Drag-and-drop is not supported; state changes happen through the task API.

### Notification Center

A bell icon in the sidebar header provides access to the notification center:

- **Unread badge** -- Displays a count of unread notifications.
- **Dropdown panel** -- Clicking the bell opens a scrollable list of recent events.
- **Event detection** -- Notifications are generated by diffing WebSocket state updates. Detected events include: worker run completed, worker run failed, new pending approval, and agent status changes.
- Notifications persist for the browser session and clear on page reload.

### Settings Page

The settings page (`/settings`) provides a full configuration editor organized into four sections:

| Section | Contents |
|---------|----------|
| **General** | Model selection dropdowns for complex, simple, and classifier task types |
| **Security** | API key management (masked display), auto-approve level toggle |
| **Scheduler** | Cron expression editing for each registered worker |
| **System Info** | Read-only display of version, Python version, platform, and data directory |

Changes are saved via `PUT /api/system/config` and take effect on the next request (leveraging hot-reloadable config).

### Keyboard Shortcuts

The following keyboard shortcuts are available from the Mission Control page:

| Key | Action |
|-----|--------|
| `H` | Switch to dashboard view |
| `C` | Switch to constellation view |
| `K` | Switch to kanban view |
| `D` | Switch to diagnostics view |
| `Esc` | Close command palette |

The command palette (opened with `Cmd+K` / `Ctrl+K`) provides fuzzy search across all available pages and actions.
...
Set this in `~/.merkaba/config.json`. When configured, all API requests must include the key as a `Bearer` token in the `Authorization` header or as an `X-API-Key` header. When not set, the dashboard is open (suitable for local-only use).

### Configuration API

Two endpoints provide programmatic access to the system configuration:

**`GET /api/system/config`** -- Returns the current configuration. API keys and sensitive values are masked in the response (e.g., `sk-ant-***`).

**`PUT /api/system/config`** -- Updates configuration values. Accepts a JSON body with the keys to update. Masked values (containing `***`) are ignored to prevent accidentally overwriting secrets with their masked representation. Changes are written to `~/.merkaba/config.json` and picked up by `HotConfig` on the next read.

Example:

```bash
# Read current config
curl -H "Authorization: Bearer $API_KEY" http://localhost:5173/api/system/config

# Update model routing
curl -X PUT -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"models": {"task_types": {"complex": "qwen3.5:122b"}}}' \
  http://localhost:5173/api/system/config
```
