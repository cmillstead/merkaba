...
class TestReviewWorker:
    def test_review_worker_approved(self):
        """LLM approves — WorkerResult.success should be True."""
        from merkaba.orchestration.review_worker import ReviewWorker

        worker = ReviewWorker()
        worker._ask_llm = MagicMock(return_value='{"approved": true, "issues": [], "suggestions": ["Consider adding tests"]}')

        task = {
            "id": 1, "name": "Review output", "task_type": "review",
            "payload": {
                "output_text": "def add(a, b): return a + b",
                "review_criteria": "Function should add two numbers",
                "task_spec": "Write an add function",
            },
        }
    ...
    def test_review_worker_rejected(self):
        """LLM rejects — issues returned in output."""
        from merkaba.orchestration.review_worker import ReviewWorker

        worker = ReviewWorker()
        worker._ask_llm = MagicMock(return_value='{"approved": false, "issues": ["Missing error handling"], "suggestions": []}')

        task = {
            "id": 2, "name": "Review output", "task_type": "review",
            "payload": {"output_text": "def div(a, b): return a/b", "review_criteria": "Handle edge cases"},
        }
        result = worker.execute(task)
        assert result.success is False
        assert "Missing error handling" in result.output["issues"]
        assert result.decisions_made[0]["decision"] == "rejected"

    def test_review_worker_clean_context(self):
        """Verify _ask_llm is called with a fresh prompt (no generation history)."""
        from merkaba.orchestration.review_worker import ReviewWorker

        worker = ReviewWorker()
        worker._ask_llm = MagicMock(return_value='{"approved": true, "issues": [], "suggestions": []}')

        task = {
            "id": 3, "name": "Review", "task_type": "review",
            "payload": {"output_text": "hello", "review_criteria": "Is greeting"},
        }
        worker.execute(task)

        # _ask_llm called once with a prompt (clean, not accumulated conversation)
        assert worker._ask_llm.call_count == 1
        prompt_arg = worker._ask_llm.call_args[0][0]
        assert "## Output to Review" in prompt_arg
        assert "## Review Criteria" in prompt_arg

    def test_review_worker_bad_json(self):
        """Malformed LLM response handled gracefully."""
        from merkaba.orchestration.review_worker import ReviewWorker

        worker = ReviewWorker()
        worker._ask_llm = MagicMock(return_value="This is not JSON at all")

        task = {
            "id": 4, "name": "Review", "task_type": "review",
            "payload": {"output_text": "code here"},
        }
        result = worker.execute(task)
        assert result.success is False
        assert result.error == "Review produced non-JSON response"
